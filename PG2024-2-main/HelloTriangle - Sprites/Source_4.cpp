...
int main()
{
	...
	Sprite background, character, background2;
	// Carregando uma textura (recebendo seu ID)

	// Inicializando a sprite do background
	int imgWidth, imgHeight;
	int texID = loadTexture("../Texturas/backgrounds/PNG/Battleground3/Bright/safeimagekit-endless.png", imgWidth, imgHeight);
	background.setupSprite(texID, vec3(400.0, 450.0, 0.0), vec3(imgWidth * 0.5, imgHeight * 0.5, 1.0), 1, 1);

    texID = loadTexture("../Texturas/backgrounds/PNG/Battleground3/Bright/grass&road.png", imgWidth, imgHeight);
	background2.setupSprite(texID, vec3(400.0, 300.0, 0.0), vec3(imgWidth * 0.5, imgHeight * 0.5, 1.0), 1, 1);

	// Inicializando a sprite do personagem
	texID = loadTexture("../Texturas/characters/PNG/Knight_running-removebg-preview.png", imgWidth, imgHeight);
